...
const getCoursesByName = async (req, res) => {
    ...
    try {
        ...
        if (courses.length === 0) {
            return res
                .status(404)
                .json({ message: "No courses found with the given name." });
        }
        res.status(200).json(courses);
    } catch (error) {
        res.status(500).json({ error: error, message: "Something went wrong" });
    ...
    try {
        ...
        if (!user) {
            return res.status(404).json({
                message:
                    "something wrong , user not found please reconnect to your account",
            });
        }
        if (!courseCategory) {
            return res.status(404).json({
                message: "Category not found , please choose an existing category",
            });
        }
...
const getCoursesByCategory = async (req, res) => {
    ...
    try {
        ...
        if (!course) {
            return res
                .status(404)
                .json({ message: "there is no course in this category" });
        }
        res.status(200).json(course);
    } catch (error) {
        res.status(500).json({ error: error, message: "Something went wrong" });
